...
<수정할 사항>
 - line 716 : html 태그 line-height 속성 설정하기
...
def keyPressed(self, e, row, col, rowCnt):
    ...
    # [참고] QTableWidget.setEditTriggers()를 설정하면 편집용(알파벳, 숫자, ... 등) 키는 
             키 이벤트 함수에서 인식하지 못함.
    '''
    movePrev = [Qt.Key_Return, Qt.Key_F9, Qt.Key_Backspace]
    moveNext = [Qt.Key_Enter, Qt.Key_F10, Qt.Key_F10]
    
    # Return, Backspace, F9 누르면 이전 문장으로 이동
    if e.modifiers() & Qt.ControlModifier or e.key() in movePrev:  

        if row > 0:
            self.view.setCurrentCell(row-1, col)
    
            tableScroll(self, row, col)
    # Enter, Shift, F10 누르면 다음 문장으로 이동        
    elif e.modifiers() & Qt.ShiftModifier or e.key() in moveNext:
    ...
    # Inseret 누르면 현재문장 복사하여 삽입(추가)
        ...
        for j in range(cols):
            ...
            if not item is None:
                temp = QTableWidgetItem(item.text())
                if j>=3:
                    temp.setTextAlignment(Qt.AlignCenter)
                self.view.setItem(curRow + 1, j, temp)
        ...
        text =("<H1 text-align:center> 도움말 보기 화면</H1>\
                <ul><li><a> 화면 아래 스크롤하기 : &lt;Ctrl/Return/Backspace/F9&gt;</a></li>\
                    <li><a> 화면 위로 스크롤하기 : &lt;Shift/Enter/F10&gt;</a></li>\
                    <li> 북마크로 이동하기    : &lt;Esc&gt;</li>\
                    <li> 북마크 표시 추가     : [암기/중요]필드에 '9';</li>\
                    <li> 현재 문장 수정하기   : &lt;F2&gt;</li>\
                    <li> 문장 추가 / 삭제하기 :[Insert/Delete];</li>\
    ...
    def run(self):
        ...
        try:
            ...
            for row in range(rows):
                ...
                sql += "(YMD, seq, korean, english, done, point) VALUES('%s', %d, '%s', '%s', '%s', '%s')" % (itm9, itm0, itm1, itm2, itm3, itm4)
