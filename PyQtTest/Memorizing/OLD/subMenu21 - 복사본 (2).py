...
class MyApp(QWidget):
    ...
    def dispTableView(self):
        ...
        self.view.setHorizontalHeaderLabels(["순", "한국어", "English", "암기", "중요", "수정"])
        ...
        try:
            ...
            for row in range(rows):
                ...
                if self.view.item(row, 0) is None:
                    continue
                if self.view.item(row, 5).text() == '0':
                    continue
                ...
                self.view.setItem(row, 5, QTableWidgetItem('0'))
                
                cursor.execute(sql)
                conn.commit()
            
            # print("Record updated sucessfully :", cursor.rowcount)
        
        except sqlite3.Error as error:
            msg = ("Failed to update data into table : %s" % error)
            QMessageBox.critical(None, "DB table Error!", msg)

        except DbException as error:
            QMessageBox.critical(None, "DB table Error!", '테이블이 존재하지 않습니다.')
            
        except Exception as error: 
            QMessageBox.critical(None, "DB file Error!", "%s" % error)
    ...
    def cell_Editing(self, row, col):    # 셀 편집 시작할 때
        pass
        # self.view.setItem(row, 5, QTableWidgetItem('1'))
        # MyApp.saveFlag = True
    ...
    def dispData2(self):
        ...
        try:
            ...
            while query.next():
                ...
                self.view.setItem(rows, 5, item6)
