...
from PyQt5.QtCore import pyqtSignal, Qt, QSize, QThread, pyqtSlot
...
class MainWindow(QMainWindow):
    ...
    def toolBarMenu(self):
        ...
        toolBtn4 = QAction(QIcon(self.imgDir + "toolBtn3.png"), 'This is your button4', self)
        toolBtn4.setStatusTip("This is your button3")
        
        toolBtn5 = QAction(QIcon(self.imgDir + "toolBtn3.png"), 'Exit program button5', self)
...
# ----- Thread object -------------------------------------------------------
def saveData_thread(*args, **kwargs):
    this = args[0]
    print(f"child thread start: args={args} kwargs={kwargs}")
    # counts = range(1, 21)
    # for row in counts:
    #     progVal = int(row/len(counts)*100) 
    #     this.progress.setValue(progVal)
    #     time.sleep(0.1)
    #     this.win.update()
    #     this.update()

    rows = this.view.rowCount()
    for row in range(rows):

        if this.view.item(row, 0) is None:
            continue

        itm0 = int(this.view.item(row, 0).text())
        itm1 = this.view.item(row, 1).text()
        itm2 = this.view.item(row, 2).text()
        itm9 = this.view.item(row, 5)
        if itm9 is None:
            itm9 = '20200101'
        # itm3 = '0'
        # itm4 = '0'
...
class App22(QTableWidget):
    ...
    def insertData(self):
        ...
        try:
            ...
            # self.thread = threading.Thread(target = saveData_thread, args=(self,), kwargs={})
            # self.thread.start()
            # self.thread.join() # 부모스레드(__main__)가 종료될 때까지 기다린다.

            conn.commit()
            # self.test.exit() # 스레드 종료
            self.saveFlag = False
            QMessageBox.information(self, " 저장 완료 ", "암기 문장을 DB 테이블에 저장했습니다.")
    ...
    def popWindow(self):
        ...
        self.pb_start = QPushButton(self.win)
        self.pb_start.setText("Start")
        self.pb_start.clicked.connect(self.thCreate)
        self.pb_start.move(10, 50)
